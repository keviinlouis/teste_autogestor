...
<script>
    ...
    import {NAME_TOKEN, URL_USUARIO} from '@/config'
        ...
        methods: {
            ...
            sendLogin(email, senha) {
                return new Promise((resolve, reject) => {
                    axios.post(URL_USUARIO + 'login', {email, senha})
                        .then((response) => {
                            localStorage.setItem(NAME_TOKEN, response.data.token);
                            localStorage.setItem('isMaster', "0");
                            localStorage.setItem('permissoes', response.data.data.permissoes);
                        })
                        .catch((error) => {
